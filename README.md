...
```bash
py -m pip install "git+https://github.com/sxhx2009-crypto/cdtw-python.git@v0.2.2"
```

최신 개발본이 필요하면 `@v0.2.2` 대신 `@main`을 사용합니다.
...
27개 단위 테스트에는 동일 곡선의 거리 0, 대칭성, 단일 셀과 다중 셀 각각의
...
- 입력 크기에는 상한이 있습니다. 비용이 높이의 **제곱**을 쓰므로 절댓값이
  `sqrt(float64 최대) ≈ 1.34e154`를 넘으면 `ValueError`를 냅니다. 예전에는
  `inf`와 함께 `RuntimeWarning`이 쏟아졌습니다.
